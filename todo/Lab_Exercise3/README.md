...
> [!IMPORTANT]
> Over the next couple of weeks, you will incrementally build an MVC-style CRUD application using Spring Boot through a series of Lab Exercises. I will not be providing a completed solution, so it is important that you build the application yourself and understand how all of its components fit together.
> For Assignment One, you will be required to build an MVC-style CRUD application with filtered internet access. It is therefore essential that you understand how to build and wire together a complete Spring Boot MVC application.

<br>

### 1. Create a Spring Boot Project in IntelliJ
...
> [!NOTE]  
> This class is annotated with `@Controller` and has an autowired reference to the `CustomerService`. The controller will handle incoming requests (through the browser)
> Autowiring in Spring Boot means Spring automatically finds and supplies an object (bean) that your class depends on. You don't have to create the object yourself with `new`.
> Spring creates an instance of `CustomerService` and injects it into the `customerService` variable in the controller.
